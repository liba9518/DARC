...
- A股：09:10，距 09:30 开盘约 20 分钟。
- 美股：21:10；夏令时距开盘约 20 分钟，冬令时距开盘约 80 分钟，因此全年都不会晚于开盘。
- 信号只使用上一完整交易日数据，不读取尚未收盘的日 K 线。

收盘后复盘闭环：

- A股 15:20：统计当天精选表现、上涨命中数、市场环境，并生成次日预备名单。
- 美股北京时间 06:30：兼容夏令时与冬令时，确保在正式收盘以后执行。
- 复盘会记录保留、新进入和暂时移出的标的。
- 次日预备名单只用于研究准备；09:10 / 21:10 盘前任务会重新计算并确认最终名单。
- 相同收盘日期和复盘结论不会重复发送。
