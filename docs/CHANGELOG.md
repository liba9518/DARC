...
- [改进] Binance 股票合约 Feishu workflow 改为运行在带 `binance-futures` 标签的 self-hosted Linux x64 runner，并新增 Binance Futures 访问预检，避免 GitHub-hosted runner 被 451 拦截后静默无推送。
- [改进] Binance 股票合约模拟开单在多空信号触发时写入独立账本，后续扫描按止盈/止损自动平仓并在飞书卡片展示胜率、胜负笔数、累计盈亏和持仓数。
- [改进] Binance 股票合约信号 GitHub Actions 新增模拟账本 cache 恢复与保存，避免云端定时扫描每轮重置胜率数据。
- [改进] Binance 股票合约飞书卡片新增模拟开单提示，按模拟账户权益、单笔风险、杠杆和最大保证金占用估算方向、数量、名义仓位和预估保证金。
- [改进] Binance 股票合约飞书卡片新增参考入场、止损、止盈1、止盈2、失效条件和单笔风险提示，明确区分观察信号与自动下单。
...
- [修复] 飞书股票策略卡片 webhook 发送新增瞬时网络错误重试，降低连接被远端重置、超时、429 或 5xx 导致推送中断的概率。
- [改进] Binance 合约做多信号推送默认改为仅在出现 `long_watch` 时发送飞书卡片，无做多信号时不再推送空状态卡。
- [改进] Binance 合约信号推送扩展为多空双向触发，默认 `--side both`，支持 `long_watch` 与 `short_watch`。

- [新功能] 新增美股/韩股独立信号推荐脚本，支持 Yahoo Finance `.KS/.KQ` 韩股候选池、KOSPI 基准和 JSON 输出。
- [新功能] 新增 Binance 合约公开行情抓取脚本，支持 U 本位/币本位合约 ticker、标记价、资金费率和 K线动量数据。
- [新功能] 新增 Binance 合约做多信号飞书推送脚本，仅推送 `long_watch`，无做多信号时可发送空信号状态卡。
- [改进] 股票策略卡片和美股/韩股信号脚本改为条件触发模式，`count=0` 时不固定推荐数量，仅输出满足条件的标的。
- [修复] yfinance 行情抓取默认使用仓库内缓存目录，避免受限用户目录导致信号源抓取失败。
- [改进] 飞书股票策略默认市场从 A股/美股切换为韩股/美股，移除 A股盘前、午间和收盘复盘策略入口并新增韩股盘前与收盘复盘链路。

- [改进] 美股策略默认扩展为五只并启用合约友好筛选配置，放宽美股流动性与一致性门槛。
- [修复] 飞书策略盘前与复盘推送在未生成有效名单时自动跳过，避免周末或数据源异常时推送空卡片。
- [新功能] 新增飞书策略卡片 GitHub Actions 云端定时推送，支持电脑关机断网后继续执行盘前、复盘和盘中监控任务。
- [改进] 飞书策略云端推送拆分关键卡片与盘中监控工作流，关键时点增加补跑并在无有效名单时发送运行状态卡。
- [改进] 飞书策略选股新增资金链路评分，结合上涨成交占优、放量承接、相对强度和回撤控制识别更适合跟随的主线标的。
- [改进] 飞书盘中监控新增无异动运行状态卡，云端正常检查但未达到提醒门槛时每天每个市场最多提示一次。
- [改进] 美股盘前策略推送窗口前移至北京时间 20:50 起多点补跑，提升 21:30 开盘前送达稳定性。
- [新功能] 飞书盘前策略和收盘复盘新增模拟跟单账本，支持 A股一万元人民币和美股一万美元模拟本金记录调仓、持仓和盈亏。
- [改进] A股盘前策略调整为北京时间 09:00、09:10、09:20 三次开盘前推送，避免开盘后仍按盘前策略补发。
- [新功能] 飞书模拟跟单新增盘中自动止盈止损纪律，支持固定止损、固定止盈和移动止盈，并在触发后推送处理卡。
- [改进] 飞书盘前策略和收盘复盘卡片新增 A股与美股大盘指数价格，便于同步判断当天市场背景。
- [改进] 飞书模拟跟单卡片新增买卖明细与持仓明细，展示买入价、卖出价、数量、金额和持仓盈亏。
- [改进] 飞书模拟跟单止盈止损处理卡补充卖出价、卖出数量和卖出金额，让模拟交易记录更直观。
- [改进] 飞书策略云端调度统一为盘前三次、收盘后三次，并新增 A股午间复盘一次。
- [改进] 飞书策略卡片接入 A股 / 美股重要快讯源，按市场关键词过滤后展示高相关资讯。
- [改进] 飞书策略新增关键卡片看门狗补推机制，发现盘前、午间或收盘复盘当天未成功记录时自动补发。
- [修复] 飞书股票策略推送支持股票专用机器人变量，避免与世界杯预测共用 Webhook 导致串群。
